...
const PICS=['https://ai-shogi-face-mitsuki.vercel.app/face.svg?v=290','https://ai-shogi-face-micchan.vercel.app/face.svg?v=290','./akiou.webp?v=2110','https://ai-shogi-face-onimama.vercel.app/face.svg?v=290','https://ai-shogi-face-mama.vercel.app/face.svg?v=290'];
const C=[
...
let ci=0,st=null,sel=null,drop=null,hist=[],thinking=false,gameOver=false,posKeys=[];
let stats=JSON.parse(localStorage.getItem('aiShogiStats210')||'{"r":1500,"w":0,"l":0,"d":0}');
function positionKey(s){const pieces=s.b.map(p=>p?(p.o===S?'s':'g')+p.k:'_').join(',');const hand=o=>['P','L','N','S','G','B','R'].map(k=>k+(s.h[o][k]||0)).join('');return(s.t===S?'S':'G')+'|'+pieces+'|'+hand(S)+'|'+hand(G)}
function keyCount(k){let n=0;for(const x of posKeys)if(x===k)n++;return n}
function recentDistance(k){for(let i=posKeys.length-1,d=1;i>=0;i--,d++)if(posKeys[i]===k)return d;return 999}
function pushPosition(){const k=positionKey(st);posKeys.push(k);return keyCount(k)}
function saveHist(){hist.push({st:E.cp(st),keys:[...posKeys]});if(hist.length>20)hist.shift()}
...
function end(result,reason=''){gameOver=true;thinking=false;if(result==='win'){stats.w++;stats.r+=Math.max(4,Math.round((C[ci].r-stats.r)/40)+16);status('あなたの勝ちです！')}else if(result==='lose'){stats.l++;stats.r-=Math.max(4,Math.round((stats.r-C[ci].r)/40)+12);status(C[ci].name+'の勝ちです。')}else{stats.d++;status(reason||'引き分けです。')}localStorage.setItem('aiShogiStats210',JSON.stringify(stats));renderStats();$('reviewBtn').textContent='対局を振り返る';setSpeech()}
function afterMove(){const k=positionKey(st);if(keyCount(k)>=4){end('draw','千日手です（同一局面が4回）。');return true}let lm=E.legal(st,st.t);if(!lm.length){if(st.t===G)end('win');else end('lose');return true}return false}
function candidateScore(s,m,cfg,side){let n=E.apply({...E.cp(s),t:side},m),k=positionKey(n),count=keyCount(k),dist=recentDistance(k),score=E.evaluate(n,side,cfg);let q=s.b[m.to];if(q)score+=(E.V[E.BASE(q.k)]||0)*.32;if(m.prom)score+=70;if(E.inCheck(n,-side))score+=95;score-=E.hangingRisk(n,side)*.45*(cfg.w?.hang||1);if(count>=3)score-=2200;else if(count>=2)score-=1400;else if(dist<=10)score-=650;return{m,n,k,count,dist,score}}
function chooseWithRepeatGuard(s,cfg,side){let pick=E.choose(s,cfg,side),search=E.getLastSearch();if(!pick)return{pick:null,avoided:false,search};let first=candidateScore(s,pick,cfg,side);if(first.count===0&&first.dist>10)return{pick,avoided:false,search};let legal=E.legal(s,side),alts=legal.map(m=>candidateScore(s,m,cfg,side)).filter(x=>x.count===0&&x.dist>10).sort((a,b)=>b.score-a.score);if(!alts.length)return{pick,avoided:false,search};let top=alts.slice(0,Math.min(5,alts.length)),quick={...cfg,time:Math.min(65,Math.max(35,Math.round(cfg.time*.08))),depth:1,root:8,branch:5,error:0};for(let a of top){let reply=E.choose({...E.cp(a.n),t:-side},quick,-side),nn=reply?E.apply(a.n,reply):a.n;a.deep=E.evaluate(nn,side,cfg);a.deep-=E.hangingRisk(nn,side)*.32*(cfg.w?.hang||1)}top.sort((a,b)=>b.deep-a.deep);let best=top[0],current=E.evaluate(s,side,cfg),margin=current<-700?110:cfg.rating>=2400?520:cfg.rating>=1900?400:300;if(best&&best.deep>=first.score-margin)return{pick:best.m,avoided:true,search};return{pick,avoided:false,search}}
function humanMove(m){saveHist();let txt=E.moveText(m,st.t,st);st=E.apply(st,m);st.log.push(txt);pushPosition();sel=drop=null;render();if(afterMove())return;status(C[ci].name+'が考えています…');thinking=true;setTimeout(aiMove,30)}
...
function aiMove(){if(gameOver)return;let cfg=E.cfgFor(C[ci]),r=chooseWithRepeatGuard(st,cfg,G),pick=r.pick;if(!pick){thinking=false;end('win');return}saveHist();let txt=E.moveText(pick,G,st);st=E.apply(st,pick);st.log.push(txt);pushPosition();thinking=false;setSpeech();render();if(afterMove())return;let si=r.search;status('あなたの手番です。　'+C[ci].name+'：'+si.depth+'手読み / '+si.nodes+'局面 / '+si.ms+'ms'+(r.avoided?' / 反復回避':''))}
...
function newGame(){st=E.fresh();posKeys=[positionKey(st)];sel=drop=null;hist=[];thinking=gameOver=false;$('shapeLabel').textContent='形勢：—';$('teacherMove').textContent='おすすめ手：解析すると表示';$('reviewSummary').textContent='あなたが1手以上指すと振り返れます。';$('reviewBtn').textContent='ここまで振り返る';setSpeech();render();status('あなたが先手です。駒をタップしてください。')}
...
$('newBtn').onclick=newGame;$('undoBtn').onclick=()=>{if(hist.length&&!thinking){let h=hist.pop();st=h.st;posKeys=h.keys;sel=drop=null;gameOver=false;render();status('待ったしました。')}};$('focusBtn').onclick=()=>{$('focus').classList.add('on');render()};$('closeBtn').onclick=()=>$('focus').classList.remove('on');$('analyzeBtn').onclick=analyze;$('reviewBtn').onclick=review;$('resetStats').onclick=()=>{stats={r:1500,w:0,l:0,d:0};localStorage.setItem('aiShogiStats210',JSON.stringify(stats));renderStats()};
newGame();window.AI_SHOGI_BUILD='2.11.0-repeat-guard';window.AI_SHOGI_DEBUG={getState:()=>E.cp(st),cfg:i=>E.cfgFor(C[i]),choose:(i=ci)=>chooseWithRepeatGuard({...E.cp(st),t:G},E.cfgFor(C[i]),G),eval:(o=S,i=ci)=>E.evaluate(st,o,E.cfgFor(C[i])),legal:()=>E.legal(st,st.t).length,rep:()=>({count:keyCount(positionKey(st)),history:posKeys.length})};
})();