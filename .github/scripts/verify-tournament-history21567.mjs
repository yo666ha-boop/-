...
assert.ok(historySource.includes('tourAttemptHistoryOrdinal21568'),'21567 history UI must expose 21568 per-cup attempt ordinal');
...
try{
  ...
  const first=await page.waitForFunction(()=>{const a=window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.();return a?.history21567&&a.historyOrdinalUi21568&&a.historyCount21567===1?a:false},{timeout:10000}).then(h=>h.jsonValue());
  assert.equal(first.historySourceCount21567,1);assert.equal(first.historyCurrent21567,1);assert.deepEqual(first.historyItems21567,['shinji']);assert.deepEqual(first.historyAttemptOrdinals21568,[1]);assert.equal(first.historyOverflow21567,0);assert.equal(first.connectors,30);assert.equal(first.roster,26);assert.equal(first.bossInBracket,false);
  const firstText=await page.locator('.tourAttemptHistory21567').innerText();assert.match(firstText,/最近の挑戦/);assert.match(firstText,/しんじ杯/);assert.match(firstText,/1回目/);assert.match(firstText,/R1500/);assert.match(firstText,/進行中/);
  ...
  await page.evaluate(()=>{window.__tourState={active:{cupId:'shinji',startedAt:6000},history:[{cupId:'shinji',startedAt:6000,rating:1720,format:'16-player-live'},{cupId:'ayanami',startedAt:5000,rating:1700,format:'16-player-live'},{cupId:'shinji',startedAt:4000,rating:1650,format:'16-player-live'},{cupId:'shinji',startedAt:3000,rating:1600,format:'16-player-live'},{cupId:'mitsuki',startedAt:2000,rating:1550,format:'16-player-live'}]};window.AI_SHOGI_TOURNAMENT_GAME_UI.render()});
  const repeat=await page.waitForFunction(()=>{const a=window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.();return a?.historySourceCount21567===5&&a.historyCount21567===3&&a.historyAttemptOrdinals21568?.[0]===3?a:false},{timeout:10000}).then(h=>h.jsonValue());
  assert.deepEqual(repeat.historyItems21567,['shinji','ayanami','shinji']);assert.deepEqual(repeat.historyAttemptOrdinals21568,[3,1,2]);assert.equal(repeat.historyCurrent21567,1);assert.equal(repeat.historyOverflow21567,0);assert.equal(repeat.docOverflow,0);assert.equal(repeat.connectors,30);assert.equal(repeat.roster,26);assert.equal(repeat.bossInBracket,false);
