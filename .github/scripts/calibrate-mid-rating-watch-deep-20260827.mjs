...
const PAIRS=[
  {upper:{i:20,name:'みさとさん',rating:1880},lower:{i:18,name:'前田慶次',rating:1820}},
  {upper:{i:18,name:'前田慶次',rating:1820},lower:{i:9,name:'あやなみ',rating:1800}},
  {upper:{i:9,name:'あやなみ',rating:1800},lower:{i:11,name:'伊達政宗',rating:1750}}
];
...
async function think(s,i){return page.evaluate(async({s,i})=>window.AI_SHOGI_YANEURAOU_COHORT13_18.bestMove(s,i),{s,i})}
async function applyMove(s,m){return page.evaluate(({s,m})=>window.__MRW.apply(s,m),{s,m})}
async function legal(s){return page.evaluate(s=>window.__MRW.legal(s),s)}
...
try{
  await page.goto('http://127.0.0.1:4239/shogi-v21528/index-mid-rating-watch.html?midwatch='+Date.now(),{waitUntil:'domcontentloaded',timeout:120000});
  await page.waitForFunction(()=>window.__MRW?.apply&&window.AI_SHOGI_YANEURAOU_FUTURE&&window.AI_SHOGI_YANEURAOU_COHORT13_18?.version==='2.15.34',{timeout:120000});
  const identity=await page.evaluate(()=>{const x=window.AI_SHOGI_YANEURAOU_COHORT13_18;return x.indices.map((i,k)=>({i:Number(i),name:String(x.names[k]),rating:Number(x.ratings[k])}))});
  ...
  const parsed=[];for(const x of STARTS)parsed.push({...x,state:await page.evaluate(pos=>window.__MRW.parse(pos),sfen(x.line))});
  const finals=[];
  for(const pair of PAIRS){
    ...
    for(const start of parsed){
      for(const upperMovesFirst of[true,false]){
        ...
        for(let ply=0;ply<MATCH_PLIES;ply++){
          const lm=await legal(s);if(!lm.length){const checked=await page.evaluate(s=>window.__MRW.incheck(s),s),turn=Number(s.t);terminal={upperCp:checked?(turn===upperSide?-MATE_CP:MATE_CP):0,kind:checked?'checkmate':'no-legal'};break}
          ...
          const token=await page.evaluate(m=>window.__MRW.usi(m),res.move);if(!lm.includes(token))throw new Error('illegal '+mover.name+' '+token);
        ...
        const row={start:start.label,startCp:start.cp,upperMovesFirst,plies:seq.length,upperCp,kind,upperMeanMoveLoss:mean(seq.filter(x=>x.mover===pair.upper.name).map(x=>x.cpLoss)),lowerMeanMoveLoss:mean(seq.filter(x=>x.mover===pair.lower.name).map(x=>x.cpLoss)),sequence:seq};games.push(row);console.log('MID_WATCH_GAME '+JSON.stringify({pair:pair.upper.rating+'-'+pair.lower.rating,...row,sequence:undefined}));
      }
    }
    ...
    const out={upper:pair.upper,lower:pair.lower,games:games.length,matchPlies:MATCH_PLIES,upperMeanMoveLoss,lowerMeanMoveLoss,moveLossAdvantage,meanPairedUpperCp,paired,verdict};finals.push(out);console.log('MID_WATCH_SUMMARY '+JSON.stringify(out));
  }
  const inversions=finals.filter(x=>x.verdict==='possible-systematic-inversion');console.log('MID_WATCH_FINAL '+JSON.stringify({pairs:finals.length,inversions:inversions.map(x=>({upper:x.upper,lower:x.lower,moveLossAdvantage:x.moveLossAdvantage,meanPairedUpperCp:x.meanPairedUpperCp})),results:finals,pageErrors}));
  if(pageErrors.length)throw new Error('page errors '+pageErrors.join(' | '));
  console.log('PASS_MID_RATING_WATCH_DEEP_DIAGNOSTIC');
