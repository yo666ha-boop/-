...
try{
  ...
  await page.waitForFunction(()=>{const a=window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.(),u=window.AI_SHOGI_TOURNAMENT_UI?.audit?.(),b=window.AI_SHOGI_TOURNAMENT_BOSS?.audit?.();return a?.hero&&a?.bossVault&&a?.connectors===30&&a?.currentMarkers===2&&u?.fit===true&&u?.rosterPortraits===26&&b?.bossInBracket===false},{timeout:10000});
  ...
  await page.waitForFunction(()=>window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.().winnerStamps>=1,{timeout:5000});
  const after=await page.evaluate(()=>window.AI_SHOGI_TOURNAMENT_GAME_UI.audit());
  assert.ok(after.winnerStamps>=1,'winner stamp missing after win');assert.equal(after.bossOutsideBracket,true);assert.equal(after.bossInBracket,false);assert.equal(after.connectors,30);assert.equal(after.docOverflow,0);
  await page.evaluate(()=>{const k='aiShogiTournament21540',s=JSON.parse(localStorage.getItem(k));s.active.status='boss_pending';s.active.bossChallenge.status='pending';localStorage.setItem(k,JSON.stringify(s));window.AI_SHOGI_TOURNAMENT.render();window.AI_SHOGI_TOURNAMENT_GAME_UI.render()});
  await page.waitForFunction(()=>window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.().progressLabel==='杯ボス解放',{timeout:5000});
  const pendingProgress=await page.evaluate(()=>window.AI_SHOGI_TOURNAMENT_GAME_UI.audit());
  await page.evaluate(()=>{const k='aiShogiTournament21540',s=JSON.parse(localStorage.getItem(k));s.active.status='boss_active';s.active.bossChallenge.status='active';localStorage.setItem(k,JSON.stringify(s));window.AI_SHOGI_TOURNAMENT.render();window.AI_SHOGI_TOURNAMENT_GAME_UI.render()});
  await page.waitForFunction(()=>window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.().progressLabel==='杯ボス挑戦中',{timeout:5000});
  const activeProgress=await page.evaluate(()=>window.AI_SHOGI_TOURNAMENT_GAME_UI.audit());
  const bossProgress={pending:pendingProgress,active:activeProgress};
  assert.equal(bossProgress.pending.progressValue,'EX');assert.equal(bossProgress.pending.progressLabel,'杯ボス解放');assert.equal(bossProgress.active.progressValue,'5戦目');assert.equal(bossProgress.active.progressLabel,'杯ボス挑戦中');assert.equal(bossProgress.active.bossOutsideBracket,true);assert.equal(bossProgress.active.connectors,30);assert.deepEqual(errors,[]);
  console.log('PASS_TOURNAMENT21559_BOSS_PROGRESS '+JSON.stringify({pending:{value:bossProgress.pending.progressValue,label:bossProgress.pending.progressLabel},active:{value:bossProgress.active.progressValue,label:bossProgress.active.progressLabel}}));
  console.log('PASS_TOURNAMENT21559_GAME_UI '+JSON.stringify({first:first.game,after,bossProgress,winnerStampGeometryOverlay:true,fireFit:first.ui.fit,rosterPortraits:first.ui.rosterPortraits,bossInBracket:first.boss.bossInBracket,pageErrors:errors}));
