...
try{
  ...
  console.log('DEBUG_TOURNAMENT21559_FIRST '+JSON.stringify(first));
  ...
  const bossProgress=await page.evaluate(()=>{const k='aiShogiTournament21540',s=JSON.parse(localStorage.getItem(k));s.active.status='boss_pending';s.active.bossChallenge.status='pending';localStorage.setItem(k,JSON.stringify(s));window.AI_SHOGI_TOURNAMENT.render();window.AI_SHOGI_TOURNAMENT_GAME_UI.render();const pending=window.AI_SHOGI_TOURNAMENT_GAME_UI.audit();s.active.status='boss_active';s.active.bossChallenge.status='active';localStorage.setItem(k,JSON.stringify(s));window.AI_SHOGI_TOURNAMENT.render();window.AI_SHOGI_TOURNAMENT_GAME_UI.render();const active=window.AI_SHOGI_TOURNAMENT_GAME_UI.audit();return{pending,active}});
