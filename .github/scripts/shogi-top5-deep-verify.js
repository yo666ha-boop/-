...
(async()=>{
  ...
  const result=await page.evaluate(async()=>{
    ...
    for(const snap of snaps){
      const refMs=snap.ply>=55?10000:7000,ref=await shared.bestMove(snap.state,{ms:refMs,multiPV:4}),cands=Array.isArray(ref?.info?.candidates)?ref.info.candidates:[],best=cands[0]||null,map=Object.fromEntries(cands.map(c=>[String(c.token||''),c]));
