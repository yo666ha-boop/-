...
function save(marker,savedAt=Date.now()){
  return {version:1,savedAt,reason:'pr93-test',ci:0,st:{b:Array(81).fill(null),h:{1:{},'-1':{}},t:1,log:[{cloudTest:marker}],last:null},hist:[],repHistory:[],gameCounted:false,lastHumanBefore:null,lastHumanMove:null,reviewTrail:[],speechMood:'normal',lastSpeech:'',statusText:'',result:null};
}
...
async function runEnv(name,type,userAgent,viewport){
  ...
  try{
    await page.goto(APP+'?pr93final='+name+'-'+RUN,{waitUntil:'domcontentloaded',timeout:120000});
    await page.waitForFunction(()=>window.crossOriginIsolated&&document.querySelectorAll('#chars .ch').length===26&&window.AI_SHOGI_SAVE&&window.AI_SHOGI_CLOUD_SAVE&&document.getElementById('saveResumeHub')&&document.getElementById('cloudSaveBtn'),null,{timeout:150000});
    ...
    const clean=await page.evaluate(async()=>{
      ...
      return {coi:crossOriginIsolated,cards:cards.length,unique:new Set(names).size,names,cloudVersion:AI_SHOGI_CLOUD_SAVE.version,raw,audit:AI_SHOGI_CLOUD_SAVE.audit(),copied,copiedText:window.__copiedFamilyCode,button:document.getElementById('cloudSaveBtn')?.textContent||'',codeText:document.getElementById('cloudCodeBtn')?.textContent||'',guide:document.getElementById('cloudSaveGuide')?.textContent||'',grouped:['cloudSaveBtn','cloudCodeBtn','cloudPullBtn'].every(id=>document.getElementById(id)?.parentElement?.id==='cloudSaveActions'),saveHub:!!document.getElementById('saveResumeHub'),overflow:document.documentElement.scrollWidth>innerWidth+1};
    });
    assert.equal(clean.coi,true);assert.equal(clean.cards,26);assert.equal(clean.unique,26);assert.equal(clean.cloudVersion,'21531f');
    assert.equal(clean.raw.familyCode,FAMILY);assert.equal(clean.raw.codeMode,'family');assert.match(clean.raw.syncKey,/^[A-Za-z0-9_-]{43}$/);
    assert.equal(clean.audit.configured,true);assert.equal(clean.copied,true);assert.equal(clean.copiedText,FAMILY);assert.equal(clean.button,'クラウド同期 ✓');assert.equal(clean.codeText,'家族コードをコピー');assert.ok(clean.guide.includes('家族コード'));assert.equal(clean.grouped,true);assert.equal(clean.saveHub,true);assert.equal(clean.overflow,false);

    await context.setOffline(true);
    await page.evaluate(v=>{localStorage.setItem('aiShogiGameSaveV1',JSON.stringify(v));window.dispatchEvent(new Event('ai-shogi-local-save'));},save(name+'-OFFLINE',RUN+1));
    ...
    assert.equal(pending.text,'同期待ち');assert.ok(pending.title.includes('未同期あり'));assert.equal(pending.meta.lastError,'');

    await context.setOffline(false);
    await page.evaluate(()=>window.dispatchEvent(new Event('online')));
    await page.waitForFunction(()=>{const m=AI_SHOGI_CLOUD_SAVE.meta();return m.revision>=1&&!m.pending&&m.lastError==='';},null,{timeout:20000});
    assert.equal(await page.locator('#cloudSaveBtn').textContent(),'クラウド同期 ✓');

    failPut=true;
    await page.evaluate(v=>{localStorage.setItem('aiShogiGameSaveV1',JSON.stringify(v));window.dispatchEvent(new Event('ai-shogi-local-save'));},save(name+'-ERROR',RUN+2));
    await page.waitForFunction(()=>AI_SHOGI_CLOUD_SAVE.meta().lastError.includes('synthetic_unavailable'),null,{timeout:20000});
    const err=await page.evaluate(()=>({text:document.getElementById('cloudSaveBtn').textContent,meta:AI_SHOGI_CLOUD_SAVE.meta()}));
    assert.equal(err.text,'同期エラー');assert.equal(err.meta.pending,true);
    failPut=false;
    const recovered=await page.evaluate(()=>AI_SHOGI_CLOUD_SAVE.push());assert.equal(recovered.ok,true);
    await page.waitForFunction(()=>{const m=AI_SHOGI_CLOUD_SAVE.meta();return !m.pending&&m.lastError==='';});
    assert.equal(await page.locator('#cloudSaveBtn').textContent(),'クラウド同期 ✓');
    assert.deepEqual(pageErrors,[]);
    console.log('PR93_PUBLIC_ENV '+JSON.stringify({name,viewport,coi:true,cards:26,unique:26,cloudVersion:'21531f',hiragana:true,clipboardExact:true,saveHub:true,grouped:true,offlinePendingText:pending.text,errorText:err.text,recovered:true,overflow:false,pageErrors}));
...
const familyCode='ぱぱ本番検証'+RUN,iphoneDevice=`pr93final_iphone_${RUN}`,pcDevice=`pr93final_pc_${RUN}`;
...
try{
  await ip.goto(APP+'?pr93mixed=i-'+RUN,{waitUntil:'domcontentloaded',timeout:120000});
  ...
  await cp.goto(APP+'?pr93mixed=p-'+RUN,{waitUntil:'domcontentloaded',timeout:120000});
  ...
  assert.equal(p0.cfg.syncKey,iState.cfg.syncKey);assert.equal(p0.cfg.familyCode,familyCode);assert.equal(await cp.locator('#cloudSaveBtn').textContent(),'クラウド同期 ✓');
  ...
  const pRev=await cp.evaluate(()=>AI_SHOGI_CLOUD_SAVE.meta().revision);assert.equal(await cp.locator('#cloudSaveBtn').textContent(),'クラウド同期 ✓');
  ...
  console.log('PR93_PUBLIC_MIXED '+JSON.stringify({iphoneInitialRevision:iState.m.revision,pcRevision:pRev,iphoneFinalRevision:fin.m.revision,cloudVersion:'21531f',realBackend:true,sameDerivedKey:p0.cfg.syncKey===iState.cfg.syncKey,hiraganaFamilyCode:true,pendingVisible:true,conflictErrorVisible:true,conflictProtected:true,explicitDiscardRestored:true,finalMarker:fin.marker,restoreLoads:fin.loads,pending:fin.m.pending,lastError:fin.m.lastError,iphoneDevice,pcDevice}));
