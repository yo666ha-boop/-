...
try{
 ...
 const rules=await page.evaluate(()=>window.AI_SHOGI_TOURNAMENT_FIELD_RULE.cups());assert.equal(rules.length,10);
 ...
 const kenshiro=rules.find(r=>r.cupId==='kenshiro'),souther=rules.find(r=>r.cupId==='souther'),akiou=rules.find(r=>r.cupId==='akiou');
 assert.equal(kenshiro.ceiling,2100);assert.equal(souther.ceiling,2180);assert.equal(akiou.ceiling,2700);assert.equal(akiou.selectedMax,2600);for(const n of ['未来からやってきたみつき','みつき','みっちゃん'])assert.ok(!akiou.selected.includes(n));
 ...
 assert.equal(first.length,16);assert.equal(first.filter(x=>x==='__PLAYER__').length,1);assert.equal(first[state.active.playerSlot],'__PLAYER__');assert.equal(new Set(first).size,16);assert.ok(!first.includes('あき王'),'cup boss must not be in the tournament bracket');
 const firstAI=first.filter(n=>n!=='__PLAYER__');assert.equal(firstAI.length,15);const firstRatings=firstAI.map(n=>ratings[names.indexOf(n)]);assert.ok(Math.max(...firstRatings)<=2700);for(const n of ['未来からやってきたみつき','みつき','みっちゃん'])assert.ok(!first.includes(n));
 ...
 const afterCount=await page.evaluate(()=>window.AIShogiIOS.characters().length);assert.equal(afterCount,26);const opponent=first[state.active.playerSlot^1],selectedIndex=await page.evaluate(()=>window.__mock.selected.at(-1));assert.equal(selectedIndex,names.indexOf(opponent));
 await page.evaluate(()=>{window.__mock.rating=1500;window.AI_SHOGI_TOURNAMENT.start('kenshiro')});state=await page.evaluate(()=>window.AI_SHOGI_TOURNAMENT.state());const kenshiroAI=state.active.bracket.rounds[0].filter(n=>n!=='__PLAYER__'),kenshiroRatings=kenshiroAI.map(n=>ratings[names.indexOf(n)]);assert.equal(kenshiroAI.length,15);assert.equal(new Set(kenshiroAI).size,15);assert.ok(!kenshiroAI.includes('ケンシロウ'));assert.ok(Math.max(...kenshiroRatings)<=2100);assert.equal(await page.evaluate(()=>JSON.stringify(window.__mock.characters)),snapshot);
 console.log('PASS_TOURNAMENT21545B_FIELD_RULE '+JSON.stringify({cups:10,entrants:16,bossSeparate:true,akiouCeiling:akiou.ceiling,akiouSelectedMax:akiou.selectedMax,kenshiroCeiling:kenshiro.ceiling,southerCeiling:souther.ceiling,highCupOverBoss:0,rosterAfter:afterCount}));
 await page.close();
