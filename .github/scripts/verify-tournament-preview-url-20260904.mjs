...
try{
  ...
  async function assertRound(round,wins,losses){
    ...
    assert.equal(ba.pairingErrors,0,'advanced character must always come from the source pair');assert.equal(ba.invalidWins,0,'blanket past-round wins must not remain');assert.ok(ba.connectors>=30,'bracket connector lines must be drawn');
    const mapping=await page.evaluate(round=>{const rs=[...document.querySelectorAll('.round')],src=[...rs[round].querySelectorAll('.slot')],dst=[...rs[round+1].querySelectorAll('.slot')];const name=x=>(x.querySelector('.name')?.textContent||'').trim();return dst.every((d,i)=>{const n=name(d);if(!n||n==='—')return true;return [name(src[i*2]),name(src[i*2+1])].includes(n)})},round);
    assert.equal(mapping,true,`round ${round} pairwise advancement`);
  }
  ...
  const finalBracketAudit=await page.evaluate(()=>window.AI_SHOGI_TOURNAMENT_BRACKET_UI.audit());assert.equal(finalBracketAudit.pairingErrors,0);assert.equal(finalBracketAudit.invalidWins,0);
  await page.close();

  const fire=await browser.newPage({viewport:{width:1280,height:800}});
  await fire.goto(url,{waitUntil:'domcontentloaded',timeout:30000});await fire.waitForFunction(()=>window.TOURNAMENT_PREVIEW_AUDIT?.().portraitCatalog===26&&window.AI_SHOGI_TOURNAMENT_BRACKET_UI,{timeout:15000});await fire.locator('[data-cup="shinji"] button').click();await fire.waitForSelector('#stage.on .bracket',{state:'visible'});const fireAudit=await fire.evaluate(()=>window.TOURNAMENT_PREVIEW_AUDIT());const fireBracket=await fire.evaluate(()=>window.AI_SHOGI_TOURNAMENT_BRACKET_UI.audit());assert.ok(fireAudit.bracketOverflow<=3,`Fire-fit preview overflow ${fireAudit.bracketOverflow}px`);assert.equal(await fire.locator('.round').count(),5);assert.equal(await fire.locator('.round:first-child .slot img').count(),15);assert.ok(fireBracket.connectors>=30);await fire.close();

  console.log('PASS_TOURNAMENT_STANDALONE_PREVIEW_21543 '+JSON.stringify({url,status,cups:8,portraitCatalog:26,firstRoundPortraits:15,initialRunning:7,initialResolved:0,fireOverflow:fireAudit.bracketOverflow,connectors:fireBracket.connectors,pairingErrors:finalBracketAudit.pairingErrors,champion:'player'}));
