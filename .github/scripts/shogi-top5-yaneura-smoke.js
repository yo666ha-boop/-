...
(async()=>{
  ...
  const expectedMaxLoss=[0,35,28,45,40];
  const failures=[];
  if(audit.version!=='2.15.32-tune5')failures.push('version '+audit.version);
  ...
  if(JSON.stringify(audit.indices.map(i=>audit.profiles[i].multiPV))!==JSON.stringify(expectedMultiPV))failures.push('multiPV '+JSON.stringify(audit.profiles));
  if(JSON.stringify(audit.indices.map(i=>audit.profiles[i].personality))!==JSON.stringify(expectedPersonalities))failures.push('personalities '+JSON.stringify(audit.profiles));
  if(JSON.stringify(audit.indices.map(i=>audit.profiles[i].maxLoss))!==JSON.stringify(expectedMaxLoss))failures.push('maxLoss '+JSON.stringify(audit.profiles));
  ...
  for(let i=0;i<5;i++){
    await cards[i].click();await page.waitForTimeout(120);
  ...
  if(engineResult?.info?.multiPV!==4)failures.push('R2500 opening MultiPV '+engineResult?.info?.multiPV);
  if(!Array.isArray(engineResult?.info?.candidates)||engineResult.info.candidates.length<2)failures.push('MultiPV candidates missing '+JSON.stringify(engineResult?.info?.candidates));
  if((engineResult?.info?.selectedRank||1)>4)failures.push('selectedRank '+engineResult?.info?.selectedRank);
  if((engineResult?.info?.cpLoss||0)>40)failures.push('cpLoss '+engineResult?.info?.cpLoss);
  if(engineResult?.info?.personality!=='stable')failures.push('personality '+engineResult?.info?.personality);

  await browser.close();
  if(failures.length)throw new Error(failures.join(' | '));
  console.log('PASS top5 tune5 + persistent game save/resume');
