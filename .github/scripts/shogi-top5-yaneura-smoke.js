...
(async()=>{
  ...
  const failures=[];
  if(audit.version!=='2.15.31-tune4')failures.push('version '+audit.version);
  ...
  if((engineResult?.info?.cpLoss||0)>65)failures.push('cpLoss '+engineResult?.info?.cpLoss);
  if(engineResult?.info?.personality!=='stable')failures.push('personality '+engineResult?.info?.personality);

  await browser.close();
  if(failures.length)throw new Error(failures.join(' | '));
  console.log('PASS top5 tune4 + persistent game save/resume');
