...
const cups={kenshiro:'ケンシロウ',souther:'サウザー',raoh:'ラオウ',kaworu:'カヲル',mama:'まま',onimama:'おにまま',akiou:'あき王',micchan:'みっちゃん',mitsuki:'みつき',future:'未来からやってきたみつき'};
...
try{
  ...
  assert.equal(voiceSamples.length,12);assert.equal(new Set(voiceSamples.map(x=>x.text)).size,12,'all twelve voice profiles must have distinct samples');assert.ok(voiceSamples.every(x=>x.speaker&&x.text&&x.total>=25));
  const portraits=[];
  for(const [cupId,boss] of Object.entries(cups)){
    const result=await page.evaluate(({cupId,boss})=>{
      const active={cupId,round:0,playerSlot:0,status:'boss_pending',pending:null,bracket:{rounds:[['__PLAYER__','ジャギ']],results:{}},bossChallenge:{status:'pending',tournamentWonAt:Date.now()-5000}};
  ...
  assert.equal(portraits.length,10);assert.equal(new Set(portraits.map(x=>x.rendered)).size,10,'each current cup boss must resolve to its own roster portrait');
  console.log('PASS_TOURNAMENT21547D_ALL_BOSS_PORTRAITS '+JSON.stringify({cups:10,voiceProfiles:12,portraitMatches:portraits.length,uniquePortraits:new Set(portraits.map(x=>x.rendered)).size,uniqueVoiceSamples:new Set(voiceSamples.map(x=>x.text)).size,minVariants:Math.min(...voiceSamples.map(x=>x.total))}));
