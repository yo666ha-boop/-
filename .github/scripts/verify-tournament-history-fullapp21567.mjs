...
try{
  ...
  const boot=async()=>{
    ...
    await page.waitForFunction(()=>window.AI_SHOGI_TOURNAMENT?.cups?.().length===10&&window.AI_SHOGI_TOURNAMENT_GAME_UI&&window.AI_SHOGI_TOURNAMENT_BRACKET_UI?.refresh&&window.__AI_SHOGI_TOURNAMENT_HISTORY_21567,null,{timeout:60000});
  };
  ...
  await page.evaluate(async()=>{
    ...
    for(const id of ['kenshiro','souther','mitsuki','future']){
  ...
  const read=async(expectedSource=4)=>{
    let lastAudit=null;
    for(let attempt=1;attempt<=5;attempt++){
      try{
        await restorePanel();
        return await page.waitForFunction(({expectedSource})=>{
          const panel=document.getElementById('tournament21540Panel');
          panel?.classList.add('on');
          window.AI_SHOGI_TOURNAMENT_BRACKET_UI?.refresh?.();
          const a=window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.();
          const history=panel?.querySelector('.tourAttemptHistory21567');
          if(!panel?.classList.contains('on')||!a?.history21567||!history||a.historySourceCount21567!==expectedSource||a.historyCount21567!==3||a.historyCurrent21567!==1||a.connectors!==30||a.roster!==26||a.bossInBracket!==false)return false;
          return {...a,historyText21567:history.innerText};
        },{expectedSource},{timeout:20000}).then(h=>h.jsonValue());
      }catch(error){
        lastAudit=await page.evaluate(()=>({audit:window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.()||null,state:window.AI_SHOGI_TOURNAMENT?.state?.()||null,history:document.querySelector('#tournament21540Panel .tourAttemptHistory21567')?.innerText||'',panel:!!document.getElementById('tournament21540Panel')?.classList.contains('on')})).catch(()=>null);
        if(attempt===5){console.error('HISTORY21592_READ_DIAGNOSTIC '+JSON.stringify({expectedSource,attempt,lastAudit}));throw error}
        if(transientNavigation(error))await page.waitForLoadState('domcontentloaded',{timeout:60000}).catch(()=>{});
  ...
  assert.deepEqual(before.historyItems21567,['future','mitsuki','souther']);
  ...
  assert.match(textBefore,/最近の挑戦/);assert.match(textBefore,/履歴 4件/);assert.match(textBefore,/未来みつき杯/);assert.match(textBefore,/みつき杯/);assert.match(textBefore,/サウザー杯/);assert.doesNotMatch(textBefore,/ケンシロウ杯/);assert.match(textBefore,/進行中/);assert.doesNotMatch(textBefore,/勝利|敗北/);

  await reloadAndRestore();
  const reloaded=await read();
  assert.deepEqual(reloaded.historyItems21567,['future','mitsuki','souther']);
  ...
  const resumeClick=await page.evaluate(()=>{
    const panel=document.getElementById('tournament21540Panel');panel?.classList.add('on');window.AI_SHOGI_TOURNAMENT?.render?.();window.AI_SHOGI_TOURNAMENT_GAME_UI?.render?.();panel?.classList.add('on');
    const item=panel?.querySelector('.tourAttemptHistoryItem21567.current[data-resumable="1"]');
    const out={found:!!item,role:item?.getAttribute('role')||'',aria:item?.getAttribute('aria-label')||'',label:item?.querySelector('.tourAttemptHistoryResume21578')?.textContent||''};item?.click();return out;
  });
  ...
  const completedBefore=await page.evaluate(()=>{
    ...
    window.AI_SHOGI_TOURNAMENT.render?.();window.AI_SHOGI_TOURNAMENT_GAME_UI?.render?.();const panel=document.getElementById('tournament21540Panel');panel?.classList.add('on');window.AI_SHOGI_TOURNAMENT_BRACKET_UI?.refresh?.();
  ...
  const retryClick=await page.evaluate(()=>{
    const panel=document.getElementById('tournament21540Panel');panel?.classList.add('on');
    const item=panel?.querySelector('.tourAttemptHistoryItem21567.current[data-retryable="1"]');
