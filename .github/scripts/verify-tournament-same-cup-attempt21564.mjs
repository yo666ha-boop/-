...
try{
  ...
  await page.evaluate(()=>{window.AI_SHOGI_TOURNAMENT.start('kenshiro');document.getElementById('tournament21540Panel')?.classList.add('on');window.AI_SHOGI_TOURNAMENT.render();window.AI_SHOGI_TOURNAMENT_GAME_UI.render()});
  const firstHandle=await page.waitForFunction(()=>{const a=window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.();const h=window.AI_SHOGI_TOURNAMENT?.state?.()?.history?.filter(x=>x?.cupId==='kenshiro').length??-1;return a?.attemptCount===1&&h===1&&a?.connectors===30&&a?.roster===26&&a?.docOverflow===0?{audit:a,history:h}:false},{timeout:10000});
  ...
  assert.equal(reloaded.activeCup,'kenshiro');assert.equal(reloaded.audit.attemptCount,2);assert.equal(reloaded.history,2);assert.equal(reloaded.audit.resumeChip,true);assert.equal(reloaded.audit.matchupCard,true);assert.equal(reloaded.audit.bossInBracket,false);assert.equal(reloaded.audit.connectors,30);assert.equal(reloaded.audit.roster,26);assert.equal(reloaded.audit.docOverflow,0);assert.deepEqual(errors,[]);

  await page.evaluate(()=>{window.AI_SHOGI_TOURNAMENT.start('souther');document.getElementById('tournament21540Panel')?.classList.add('on');window.AI_SHOGI_TOURNAMENT.render();window.AI_SHOGI_TOURNAMENT_GAME_UI.render()});
  const southerHandle=await page.waitForFunction(()=>{const a=window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.();const s=window.AI_SHOGI_TOURNAMENT?.state?.();const kenshiro=s?.history?.filter(x=>x?.cupId==='kenshiro').length??-1;const souther=s?.history?.filter(x=>x?.cupId==='souther').length??-1;return s?.active?.cupId==='souther'&&a?.attemptCount===1&&kenshiro===2&&souther===1&&a?.connectors===30&&a?.roster===26&&a?.docOverflow===0?{audit:a,kenshiro,souther}:false},{timeout:10000});
  const souther=await southerHandle.jsonValue();
  assert.equal(souther.audit.attemptCount,1);assert.equal(souther.kenshiro,2);assert.equal(souther.souther,1);assert.equal(souther.audit.resumeChip,false,'new cup after reload must not show resume chip');

  await page.evaluate(()=>{window.AI_SHOGI_TOURNAMENT.start('kenshiro');document.getElementById('tournament21540Panel')?.classList.add('on');window.AI_SHOGI_TOURNAMENT.render();window.AI_SHOGI_TOURNAMENT_GAME_UI.render()});
  const thirdKenshiroHandle=await page.waitForFunction(()=>{const a=window.AI_SHOGI_TOURNAMENT_GAME_UI?.audit?.();const s=window.AI_SHOGI_TOURNAMENT?.state?.();const kenshiro=s?.history?.filter(x=>x?.cupId==='kenshiro').length??-1;const souther=s?.history?.filter(x=>x?.cupId==='souther').length??-1;return s?.active?.cupId==='kenshiro'&&a?.attemptCount===3&&kenshiro===3&&souther===1&&a?.connectors===30&&a?.roster===26&&a?.docOverflow===0?{audit:a,kenshiro,souther}:false},{timeout:10000});
  const thirdKenshiro=await thirdKenshiroHandle.jsonValue();
  assert.equal(thirdKenshiro.audit.attemptCount,3);assert.equal(thirdKenshiro.kenshiro,3);assert.equal(thirdKenshiro.souther,1);assert.equal(thirdKenshiro.audit.resumeChip,false,'new same-cup start after reload must not show resume chip');assert.deepEqual(errors,[]);
  ...
  console.log('PASS_TOURNAMENT21566_PER_CUP_ATTEMPT_ISOLATION '+JSON.stringify({souther:{attemptCount:souther.audit.attemptCount,kenshiroHistory:souther.kenshiro,southerHistory:souther.souther,resumeChip:souther.audit.resumeChip},kenshiroThird:{attemptCount:thirdKenshiro.audit.attemptCount,kenshiroHistory:thirdKenshiro.kenshiro,southerHistory:thirdKenshiro.souther,resumeChip:thirdKenshiro.audit.resumeChip},pageErrors:errors}));
