...
try{
  ...
  await page.evaluate(()=>{const t=window.AI_SHOGI_TOURNAMENT;if(t.state()?.active)t.exit();if(!t.start('kenshiro'))throw new Error('start failed');t.render?.();window.AI_SHOGI_TOURNAMENT_GAME_UI?.render?.();window.AI_SHOGI_TOURNAMENT_BRACKET_UI?.refresh?.()});
  await page.waitForFunction(()=>{window.AI_SHOGI_TOURNAMENT?.render?.();window.AI_SHOGI_TOURNAMENT_GAME_UI?.render?.();window.AI_SHOGI_TOURNAMENT_BRACKET_UI?.refresh?.();return document.querySelectorAll('#tournament21540Panel .tourRoadStage21562').length===5&&document.querySelectorAll('#tournament21540Panel .tourBracketLines path').length===30},{timeout:30000});
  ...
  const setState=async (patch,expect)=>page.evaluate(async ({p,e})=>{
    ...
    while(Date.now()<deadline){window.AI_SHOGI_TOURNAMENT?.render?.();window.AI_SHOGI_TOURNAMENT_GAME_UI?.render?.();window.AI_SHOGI_TOURNAMENT_BRACKET_UI?.refresh?.();await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));const row=snap();if(JSON.stringify(row.labels)===JSON.stringify(['1R','QF','SF','F','EX'])&&row.done===e.done&&row.current===e.current&&row.failed===e.failed&&row.connectors===30)return row;}
    return snap();
  },{p:patch,e:expect});
  const cases=[];
