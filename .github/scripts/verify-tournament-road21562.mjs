...
try{
  ...
  await page.evaluate(()=>{const k='aiShogiTournament21540';localStorage.setItem(k,JSON.stringify({active:{cupId:'shinji',round:0,status:'active',bossChallenge:{status:'locked'}}}));window.__AI_SHOGI_TOURNAMENT_ROAD_21562?.render?.()});
  ...
  const setState=async (patch,expect)=>{
    await page.evaluate(p=>{const k='aiShogiTournament21540',s=JSON.parse(localStorage.getItem(k)||'null');if(!s?.active)throw new Error('no active');Object.assign(s.active,p.active||{});s.active.bossChallenge={...(s.active.bossChallenge||{}),...(p.bossChallenge||{})};localStorage.setItem(k,JSON.stringify(s));window.__AI_SHOGI_TOURNAMENT_ROAD_21562?.render?.()},patch);
