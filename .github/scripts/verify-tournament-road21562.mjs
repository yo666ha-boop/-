...
try{
  ...
  await page.evaluate(()=>{const t=window.AI_SHOGI_TOURNAMENT;if(t.state()?.active)t.exit();if(!t.start('shinji'))throw new Error('start failed')});
  ...
  const setState=async patch=>{
    await page.evaluate(p=>{const k='aiShogiTournament21540',s=JSON.parse(localStorage.getItem(k)||'null');if(!s?.active)throw new Error('no active');Object.assign(s.active,p.active||{});s.active.bossChallenge={...(s.active.bossChallenge||{}),...(p.bossChallenge||{})};localStorage.setItem(k,JSON.stringify(s))},patch);
    await page.reload({waitUntil:'domcontentloaded',timeout:60000});
    await page.waitForFunction(()=>window.__AI_SHOGI_TOURNAMENT_ROAD_21562&&document.querySelectorAll('#tournament21540Panel .tourRoadStage21562').length===5,{timeout:60000});
  ...
  for(const [name,patch,expect] of [
    ...
    const row=await setState(patch); const bad=[];
