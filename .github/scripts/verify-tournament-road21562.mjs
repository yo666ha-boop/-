...
try{
  ...
  const setState=async patch=>{
    await page.evaluate(p=>{const k='aiShogiTournament21540',s=JSON.parse(localStorage.getItem(k)||'null');if(!s?.active)throw new Error('no active');Object.assign(s.active,p.active||{});s.active.bossChallenge={...(s.active.bossChallenge||{}),...(p.bossChallenge||{})};localStorage.setItem(k,JSON.stringify(s));window.dispatchEvent(new Event('ai-shogi-local-save'));window.AI_SHOGI_TOURNAMENT?.render?.();window.AI_SHOGI_TOURNAMENT_GAME_UI?.render?.()},patch);
    await page.waitForFunction(()=>window.__AI_SHOGI_TOURNAMENT_ROAD_21562&&document.querySelectorAll('#tournament21540Panel .tourRoadStage21562').length===5,{timeout:10000});
