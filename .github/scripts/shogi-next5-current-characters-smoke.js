...
(async()=>{
  ...
  const url='http://127.0.0.1:8000/shogi-v21528/?next5current='+Date.now();
  ...
  const audit=await page.evaluate(()=>{
    ...
    return{
      prepVersion:prep.version,enabled:prep.enabled,liveOverride:prep.liveOverride,
      targets:prep.targets,roster:prep.verifyRoster(),
      actualTargets:prep.targets.map(i=>({i,name:chars[i]?.name,rating:chars[i]?.rating,style:chars[i]?.style})),
      top5Version:top5.version,top5Indices:top5.indices,
      top5MaxLoss:top5.indices.map(i=>top5.profiles[i].maxLoss),coi:crossOriginIsolated
    };
  });
  console.log('NEXT5_CURRENT_AUDIT',JSON.stringify(audit));
  const failures=[];
  const expectedTargets=[24,23,21,5,17];
  const expectedNames=['カヲル','ラオウ','サウザー','ケンシロウ','げんどー'];
  const expectedRatings=[2400,2250,2180,2100,2050];
  if(audit.prepVersion!=='2.15.33-prep-correct1')failures.push('prep version '+audit.prepVersion);
  ...
  if(JSON.stringify(audit.actualTargets.map(x=>x.name))!==JSON.stringify(expectedNames))failures.push('names '+JSON.stringify(audit.actualTargets));
  if(JSON.stringify(audit.actualTargets.map(x=>x.rating))!==JSON.stringify(expectedRatings))failures.push('ratings '+JSON.stringify(audit.actualTargets));
  ...
  const probes=await page.evaluate(async()=>{
    ...
    for(const who of api.targets){
      ...
      out.push({who:r.who,name:r.name,rating:r.rating,ms:r.ms,multiPV:r.multiPV,bestToken:r.bestToken,engine:r.info?.engine||'',candidates:r.candidates});
    }
    return out;
  });
  console.log('NEXT5_CURRENT_PROBES',JSON.stringify(probes));
  ...
  console.log('PASS correct current R6-R10 characters; Suisho5 probe works; live override OFF; top5 tune5 unchanged');
