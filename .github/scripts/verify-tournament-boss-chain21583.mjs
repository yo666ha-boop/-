...
try{
  ...
  const report=await page.evaluate(async()=>{
    ...
    await reachBoss(); await delay(80);
    const pending=t.state().active, pendingAudit=window.AI_SHOGI_TOURNAMENT_BOSS.audit(), pendingButton=document.getElementById('tournament21540Btn')?.textContent?.trim()||'';
    const trophyBefore=Number(t.state().trophies?.kenshiro||0);
    if(!t.challengeBoss())throw Error('boss challenge'); if(!await wait('active'))throw Error('boss active timeout'); await delay(80);
    const activeButton=document.getElementById('tournament21540Btn')?.textContent?.trim()||'';
    await result('draw'); if(!await wait('draw'))throw Error('boss draw timeout'); await delay(80);
    const drawButton=document.getElementById('tournament21540Btn')?.textContent?.trim()||'';
    if(!t.challengeBoss())throw Error('boss retry'); if(!await wait('active'))throw Error('boss retry active timeout');
    await result('loss'); if(!await wait('lost'))throw Error('boss lost timeout'); await delay(80);
    const loss=t.state().active, lossButton=document.getElementById('tournament21540Btn')?.textContent?.trim()||'', trophyAfterLoss=Number(t.state().trophies?.kenshiro||0);
    ...
    await result('win'); if(!await wait('won'))throw Error('boss won timeout'); await delay(80);
    const won=t.state().active, wonButton=document.getElementById('tournament21540Btn')?.textContent?.trim()||'', trophyAfterWin=Number(t.state().trophies?.kenshiro||0);
    return {expected,cups:t.cups().map(c=>[c.id,c.boss,c.bossRating]),fields,ordersUnique:new Set(orders).size,pendingStatus:pending?.status,pendingBoss:pending?.bossChallenge?.status,pendingChampion:pending?.bracket?.rounds?.[4]?.[0],pendingAudit,pendingButton,activeButton,drawButton,trophyBefore,lossStatus:loss?.status,lossBoss:loss?.bossChallenge?.status,lossButton,trophyAfterLoss,wonStatus:won?.status,wonBoss:won?.bossChallenge?.status,wonButton,trophyAfterWin};
  });
  const failures=[];
  ...
  for(const [phase,label] of [['pending',report.pendingButton],['active',report.activeButton],['draw',report.drawButton]])if(!String(label).includes('大会表を開く'))failures.push(phase+' button '+JSON.stringify(label));
  ...
  if(!String(report.lossButton).includes('大会結果を開く'))failures.push('loss button '+JSON.stringify(report.lossButton));
  if(report.wonBoss!=='won'||report.trophyAfterWin!==report.trophyBefore+1)failures.push('win semantics');
  if(!String(report.wonButton).includes('大会結果を開く'))failures.push('win button '+JSON.stringify(report.wonButton));
