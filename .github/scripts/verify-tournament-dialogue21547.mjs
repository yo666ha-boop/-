...
try{
  ...
  audit=await page.evaluate(()=>window.AI_SHOGI_TOURNAMENT_DIALOGUE.audit());assert.equal(audit.context,'boss_mid');assert.equal(audit.label,'杯ボス戦・中盤');assert.equal(audit.scoreSource,'material');
  ...
  console.log('PASS_TOURNAMENT21547C_DIALOGUE '+JSON.stringify({bosses:bankAudit.bosses,contexts:bankAudit.contexts,minVariants:bankAudit.minVariants,totalContextVariants:bankAudit.totalContextVariants,antiRepeatUnique:new Set(anti).size,qfPortrait:true,qfRole:'host-outside-bracket',bossInBracket:false,bossMid:true,materialFallbackAdvantage:true,materialFallbackDisadvantage:true,teacherEvalPriority:true,completeClear:true,mobileOverflow:Math.max(0,mobile.doc-mobile.vw)}));
