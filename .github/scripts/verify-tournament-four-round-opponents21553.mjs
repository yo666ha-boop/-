...
try{
 ...
 const rows=await page.evaluate(async()=>{
  ...
  const snap=()=>{d.render();const a=t.state()?.active,row=a?.bracket?.rounds?.[a?.round],opp=row?.[(Number(a?.playerSlot)||0)^1]||'',box=document.getElementById('tourOpponentVoice21549'),img=box?.querySelector('img'),r=box?.getBoundingClientRect?.()||{},baseSpeech=(document.getElementById('charSpeech')?.textContent||'').trim();return{round:Number(a?.round),expectedOpponent:opp,speaker:box?.dataset.speaker||'',role:box?.dataset.role||'',actual:canon(img?.currentSrc||img?.src||''),expected:cardSrc(opp),complete:!!img?.complete,w:Number(img?.naturalWidth)||0,h:Number(r.height)||0,docked:box?.classList.contains('tourRoundBattleDock21550')===true,parent:box?.parentElement?.classList?.contains('side')?'side':'other',text:(box?.querySelector('.tourDialogueBubble')?.textContent||'').trim(),baseSpeech};};
  if(t.state()?.active)t.exit();if(!t.start('kenshiro'))throw new Error('start');await delay(3500);out.push(snap());
  for(let round=0;round<3;round++){await result();t.next();await delay(250);out.push(snap())}
  t.exit();return out;
 });
 const f=[];for(const x of rows){if(!x.expectedOpponent||x.speaker!==x.expectedOpponent||x.role!=='対戦相手・トーナメント参加者'||!x.actual||x.actual!==x.expected||!x.complete||x.w<1||x.h<1||!x.docked||x.parent!=='side'||!x.text)f.push(JSON.stringify(x));if(x.speaker!=='みっちゃん'&&(!x.baseSpeech||x.text!==x.baseSpeech))f.push('voice inheritance '+JSON.stringify(x))}
 const inherited=rows.filter(x=>x.speaker!=='みっちゃん'&&x.baseSpeech&&x.text===x.baseSpeech);
 if(rows.length!==4||new Set(rows.map(x=>x.speaker)).size!==4)f.push('round/opponent uniqueness '+JSON.stringify(rows.map(x=>x.speaker)));if(inherited.length<3)f.push('insufficient live voice inheritance samples '+JSON.stringify(rows));if(errs.length)f.push('errors '+JSON.stringify(errs));if(f.length)throw new Error(f.join('|'));
 console.log('PASS_TOURNAMENT21553_FOUR_ROUND_OPPONENT_PORTRAITS '+JSON.stringify({rows,pageErrors:errs}));
 console.log('PASS_TOURNAMENT21598_FULLAPP_OPPONENT_VOICE_INHERITANCE '+JSON.stringify({sampled:rows.length,inherited:inherited.length,onlyOverride:'みっちゃん',rows:rows.map(x=>({round:x.round,speaker:x.speaker,text:x.text,baseSpeech:x.baseSpeech,equal:x.speaker==='みっちゃん'?null:x.text===x.baseSpeech})),pageErrors:errs}));
