...
async function runCase(browserType,label,contextOptions={}){
  ...
  await page.waitForFunction(()=>document.getElementById('cloudFamilySwitchBtn'));await page.locator('#cloudFamilySwitchBtn').click();await page.getByRole('button',{name:'現在の家族コードの保存を見る'}).click();await page.waitForFunction(()=>document.querySelector('[aria-label="家族コードの保存を選ぶ"]'));
