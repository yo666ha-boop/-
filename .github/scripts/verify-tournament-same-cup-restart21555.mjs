...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const currentOpponent=a=>{
      const row=a?.bracket?.rounds?.[a?.round];
      return Array.isArray(row)?row[(Number(a.playerSlot)||0)^1]||null:null;
    };
    const snap=label=>{
      d.render();
      const a=t.state()?.active||null,box=document.getElementById('tourDialogue21547'),img=box?.querySelector('img'),r=box?.getBoundingClientRect?.()||{},opp=document.getElementById('tourOpponentVoice21549'),oppImg=opp?.querySelector('img'),or=opp?.getBoundingClientRect?.()||{},side=document.querySelector('.side');
      return{
        ...
        expectedOpponent:currentOpponent(a)||'',opponentSpeaker:opp?.dataset.speaker||'',opponentRole:opp?.dataset.role||'',
        opponentText:(opp?.querySelector('.tourDialogueBubble')?.textContent||'').trim(),
        opponentPortrait:!!oppImg?.src&&!!oppImg.complete&&oppImg.naturalWidth>0,
        docked:box?.classList.contains('tourRoundBattleDock21550')===true,
        opponentDocked:opp?.classList.contains('tourRoundBattleDock21550')===true,
        parent:box?.parentElement?.classList?.contains('side')?'side':'other',
        opponentParent:opp?.parentElement?.classList?.contains('side')?'side':'other',
        h:Math.round(r.height||0),opponentH:Math.round(or.height||0),sideOverflow:side?Math.max(0,side.scrollWidth-side.clientWidth):0,
  ...
  for(const x of [result.first,result.second]){
    ...
    if(x.speaker!=='ケンシロウ'||x.role!=='大会主・トーナメント外'||!x.lineId||!x.text||!x.portrait)f.push(x.label+': host dialogue '+JSON.stringify(x));
    if(!x.expectedOpponent||x.opponentSpeaker!==x.expectedOpponent||x.opponentRole!=='対戦相手・トーナメント参加者'||!x.opponentText||!x.opponentPortrait)f.push(x.label+': opponent dialogue '+JSON.stringify(x));
    if(!x.docked||!x.opponentDocked||x.parent!=='side'||x.opponentParent!=='side'||x.h!==0||x.opponentH<1||x.opponentH>115||x.sideOverflow!==0||x.docOverflow!==0)f.push(x.label+': layout '+JSON.stringify(x));
  }
  ...
  console.log('PASS_TOURNAMENT21555_SAME_CUP_RESTART_OPPONENT '+JSON.stringify({...result,pageErrors:errors}));
}finally{
  await browser.close();
}
