...
try{
 ...
 const r=await page.evaluate(async()=>{
  const K='aiShogiTournament21540',H='aiShogiTournamentDialogue21547',cupId='kenshiro',t=window.AI_SHOGI_TOURNAMENT,d=window.AI_SHOGI_TOURNAMENT_DIALOGUE,delay=ms=>new Promise(x=>setTimeout(x,ms));
  if(t.state()?.active)t.exit();if(!t.start(cupId))throw new Error('start');await delay(200);
  ...
  const snap=()=>{const b=document.getElementById('tourDialogue21547'),i=b?.querySelector('img'),x=d.audit();let h={};try{h=JSON.parse(localStorage.getItem(H)||'{}')}catch{};return{context:x.context,lineId:b?.dataset.lineId||'',speaker:x.speaker,role:b?.dataset.role||'',status:(b?.querySelector('.tourDialogueStatus')?.textContent||'').trim(),text:(b?.querySelector('.tourDialogueBubble')?.textContent||'').trim(),src:i?.currentSrc||i?.src||'',complete:!!i?.complete,w:Number(i?.naturalWidth)||0,hist:(h.byKey?.[cupId+':upset']||[]).slice()}};const one=snap();d.render();await delay(80);const two=snap(),boss=t.cups().find(c=>c.id===cupId).boss,card=[...document.querySelectorAll('#chars .ch')].find(c=>(c.querySelector('.chName')?.textContent||c.querySelector('img')?.alt||'').trim()===boss),ri=card?.querySelector('img'),expected=ri?.currentSrc||ri?.src||'';t.exit();return{p,boss,expected,one,two}
 });
