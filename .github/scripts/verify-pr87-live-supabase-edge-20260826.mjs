...
x=await req('GET',code);assert.equal(x.r.status,200);assert.equal(x.j.record.revision,1);
...
x=await req('GET',other);assert.equal(x.r.status,200);assert.equal(x.j.record.payload.st.log[0],'OTHER');
x=await req('GET','',undefined);assert.equal(x.r.status,401);assert.equal(x.j.error,'invalid_sync_key');
x=await req('GET',code,undefined,'https://evil.example');assert.equal(x.r.status,403);assert.equal(x.j.error,'origin_not_allowed');
x=await req('DELETE',code);assert.equal(x.r.status,405);assert.equal(x.j.error,'method_not_allowed');
