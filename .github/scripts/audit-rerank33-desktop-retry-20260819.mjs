...
try{
  ...
  const out=await page.evaluate(async()=>{
    ...
    w.onmessage=e=>{const m=e.data||{};if(m.type==='stage'){stages.push(m.text);return}const p=P.get(m.id);if(!p||m.type!=='result')return;P.delete(m.id);clearTimeout(p.t);m.ok?p.r(m):p.j(Error(m.error||'worker'))};
    const call=(type,x={},timeout=180000)=>new Promise((r,j)=>{const n=++id,t=setTimeout(()=>{P.delete(n);j(Error(type+' timeout '+timeout+' stages='+stages.slice(-12).join(' | ')))},timeout);P.set(n,{r,j,t});w.postMessage({type,id:n,...x})});
    try{
      const startedInit=Date.now(),init=await call('init',{},180000),initElapsed=Date.now()-startedInit,started=Date.now(),r=await call('bestmove',{sfen:'lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL b - 1',ms:700,multiPV:1},60000);
      return{ok:true,coi:crossOriginIsolated,cards:document.querySelectorAll('#chars .ch').length,init,r,initElapsed,elapsed:Date.now()-started,stages:stages.slice(-16)};
    }catch(e){return{ok:false,coi:crossOriginIsolated,cards:document.querySelectorAll('#chars .ch').length,error:String(e?.message||e),stages:stages.slice(-16)}}finally{w.terminate()}
  });
  console.log('DESKTOP_RETRY_DIAG '+JSON.stringify(out));
  if(!out.ok)throw Error('desktop init/search failed '+out.error+' stages='+JSON.stringify(out.stages));
