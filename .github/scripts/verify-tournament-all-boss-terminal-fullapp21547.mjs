...
try{
  ...
  const snapReload=async(label,force=false)=>await page.evaluate(({label,force})=>{
    ...
    const assetIdentity=src=>{if(!src)return'';if(src.startsWith('data:'))return src;try{const u=new URL(src,location.href);return u.origin+u.pathname}catch(e){return String(src).split('?')[0]}};
    const actualAsset=assetIdentity(actual),expectedAsset=assetIdentity(expected);
    ...
    return{label,context:da.context||null,lineId:box?.dataset.lineId||null,speaker:da.speaker||null,role:box?.dataset.role||null,boss:cup.boss,bossStatus:t.state()?.active?.bossChallenge?.status||null,portraitMatch:!!expectedAsset&&actualAsset===expectedAsset,portraitActual:actual,portraitExpected:expected,portraitActualAsset:actualAsset,portraitExpectedAsset:expectedAsset,visible:!!box&&getComputedStyle(box).display!=='none'&&rect.width>0&&rect.height>0,dock:dock?.audit?.()||null,oppName:(document.getElementById('oppName')?.textContent||'').trim(),historyIds:ids,historySentinel:sentinel,restoreAudit:window.AI_SHOGI_TOURNAMENT_RELOAD_RESTORE?.audit?.()||null};
  },{label,force});
  ...
  for(const [beforeKey,afterKey,context] of reloadPairs){
    ...
    if(!before.portraitMatch||!after.portraitMatch||!before.visible||!after.visible)failures.push(`${afterKey}: portrait/visibility restore mismatch actual=${after.portraitActualAsset} expected=${after.portraitExpectedAsset}`);
  ...
  console.log('PASS_TOURNAMENT21547F_RELOAD_RESTORE '+JSON.stringify({contexts:reloadPairs.map(([,afterKey])=>restore[afterKey].context),freshStartGuard:report.initialRestoreAudit?.hadInitialActive===false&&report.initialRestoreAudit?.done===true,historySentinelPersisted:reloadPairs.filter(([,afterKey])=>restore[afterKey].historySentinel).length,antiRepeatAfterRestore:Object.values(anti).filter(x=>x.before&&x.after&&x.before!==x.after).length,pendingRestored:restore.pendingAfter.bossStatus==='pending'&&!restore.pendingAfter.dock?.docked,activeRestored:restore.activeAfter.bossStatus==='active'&&restore.activeAfter.dock?.docked,drawRestored:restore.drawAfter.bossStatus==='draw'&&!restore.drawAfter.dock?.docked,retryRestored:restore.retryAfter.bossStatus==='active'&&restore.retryAfter.dock?.docked,bossOpponentRestored:[restore.activeAfter,restore.retryAfter].every(x=>x.oppName.startsWith(x.boss)),reloadPortraitAssets:reloadPairs.map(([,afterKey])=>restore[afterKey].portraitActualAsset),pageErrors}));
} finally {
  await browser.close();
}