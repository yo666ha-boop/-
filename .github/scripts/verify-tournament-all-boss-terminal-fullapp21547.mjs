...
try{
  ...
  for(const [bk,ak,ctx] of pairs){const b=restore[bk],a=restore[ak];if(b.context!==ctx||a.context!==ctx)failures.push(`${ak}: context ${b.context}/${a.context}`);if(b.speaker!==b.boss||a.speaker!==a.boss)failures.push(`${ak}: speaker`);if(!b.portraitMatch||!a.portraitMatch||!b.visible||!a.visible)failures.push(`${ak}: portrait/visibility actual=${a.actualAsset} expected=${a.expectedAsset}`);if(!a.historySentinel)failures.push(`${ak}: history sentinel`);if(a.restoreAudit?.hadInitialActive!==true||a.restoreAudit?.done!==true)failures.push(`${ak}: restore audit`)}
