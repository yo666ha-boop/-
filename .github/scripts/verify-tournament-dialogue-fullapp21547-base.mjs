...
try{
  ...
  for(const label of ['round1','next1','next2','next3']){const x=by[label];if(!x?.opponentSpeaker||x.opponentRole!=='対戦相手・トーナメント参加者'||!x.opponentPortrait||!x.opponentText)transitionFailures.push(label+': participant dialogue missing');else if(!x.roundDocked||x.roundDockParent!=='side'||x.panelOpen)transitionFailures.push(label+': round dialogue not docked beside board '+JSON.stringify({roundDocked:x.roundDocked,parent:x.roundDockParent,panelOpen:x.panelOpen}))}
