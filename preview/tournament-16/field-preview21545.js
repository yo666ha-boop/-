/* みつき将棋 大会参加者ルール・外部確認ページ 21545b */
(function installPreviewField21545(){
  'use strict';
  if(window.__TOURNAMENT_PREVIEW_FIELD_21545B)return;
  window.__TOURNAMENT_PREVIEW_FIELD_21545B=true;
  ...
  function rule(cup){
    ...
    const selected=nonBoss.filter(ch=>rate(ch)<=ceiling).sort((a,b)=>score(a)-score(b)||rate(a)-rate(b)||String(a.name).localeCompare(String(b.name),'ja')).slice(0,15);
  ...
  roster=function(cup){const r=rule(cup);return[PLAYER,...r.selected]};
  window.TOURNAMENT_PREVIEW_FIELD_RULE={
    version:'21545b',
    audit:(id='akiou')=>{const cup=CUPS.find(c=>c.id===id)||CUPS.find(c=>c.id==='akiou');return{ok:true,...rule(cup),bossSeparate:true,bossInBracket:false}},
    cups:()=>CUPS.map(rule)
  };
})();