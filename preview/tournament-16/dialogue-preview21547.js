/* みつき将棋 外部確認ページ・画像付き状況セリフ 21547b */
(function installPreviewDialogue21547(){
  'use strict';
  if(window.__TOURNAMENT_PREVIEW_DIALOGUE_21547B)return;
  window.__TOURNAMENT_PREVIEW_DIALOGUE_21547B=true;
  ...
  function derive(){
    ...
    if(bs==='pending')ctx='boss_pending';else if(bs==='active'){const raw=(document.getElementById('evalNumber')?.textContent||'').trim(),score=raw&&raw!=='—'?Number(raw.replace(/[^+\-\d.]/g,'')):null;ctx=Number.isFinite(score)&&score>=350?'boss_advantage':Number.isFinite(score)&&score<=-350?'boss_disadvantage':'boss_start'}else if(bs==='won')ctx='boss_won';else if(bs==='lost')ctx='boss_lost';else if(bs==='draw')ctx='boss_draw';
...
`;document.head.appendChild(s)}
  ...
  window.TOURNAMENT_DIALOGUE_PREVIEW_AUDIT=()=>{const el=document.getElementById('dialoguePreview21547'),b=bank()?.audit?.()||{};return{ok:!!b.ok&&(!active||!!el?.querySelector('img')),version:'21547b',bank:b,context:el?.dataset.context||null,speaker:el?.dataset.speaker||null,role:el?.dataset.role||null,portrait:!!el?.querySelector('img')?.src,lineId:el?.dataset.lineId||null,label:el?.querySelector('.dpStatus')?.textContent||'',text:el?.querySelector('.dpSpeech')?.textContent||'',overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth}};
