...
`;document.head.appendChild(s)}
  ...
  function render(){style();const d=derive(),el=box();if(!d){el.style.display='none';return false}const p=pick(d);if(!p)return false;const src=IMG[d.cup.boss]||'',role=['pending','active','won','lost','draw'].includes(active.bossChallenge?.status)?'杯ボス':'大会主・トーナメント外';el.style.display='grid';el.dataset.context=d.ctx;el.dataset.speaker=d.cup.boss;el.dataset.role=role;el.dataset.lineId=p.id;el.innerHTML='<div class="dpImg">'+(src?'<img src="'+esc(src)+'" alt="'+esc(d.cup.boss)+'">':'')+'</div><div><div class="dpTop"><span class="dpStatus">'+esc(p.label)+'</span><span class="dpRole">'+esc(role)+'</span></div><div class="dpName">'+esc(d.cup.boss)+'</div><div class="dpSpeech">'+esc(p.text)+'</div></div>';return true}
