 ...
 * 対局中は既存の正規「対戦相手」カードを主役にし、杯ボスの大会主コメントと混同させない。
...
(function installTournamentVisual21590(){
  ...
  const OPPONENT_LINES={
    'みつき':{reveal:['ここまで来たね。次は私と勝負だよ。','次は私の番。最後まで読んでいくよ。']},
    'みっちゃん':{reveal:['次はみっちゃんだよ。思いっきりいこう！','待ってたよ。次は私と勝負！']},
    'あき王':{reveal:['次は僕だ。落ち着いて一局いこう。','次の相手は僕だよ。丁寧に指そう。']},
    'おにまま':{reveal:['次はあたしだよ。甘い手は見逃さないよ。','ここからはあたしが相手だよ。本気で来な。']},
    'まま':{reveal:['次はままと一局ね。落ち着いていこう。','次は私ね。焦らず指そうね。']},
    'ケンシロウ':{reveal:['次は俺だ。自分の読みを信じろ。','ここからは俺が相手だ。最後まで指し切れ。']},
    'ジャギ':{reveal:['次は俺の番だ。簡単には通さないぞ。','ここから先は俺が相手だ。']},
    'しんじ':{reveal:['次はぼくだね。よろしく。','ぼくが次の相手だね。落ち着いて指そう。']},
    '直江兼続':{reveal:['次は私が相手です。正面から受けます。','次局、お相手します。丁寧にいきましょう。']},
    'あやなみ':{reveal:['次は私と指す。','次の対局、私が相手。']},
    'バット':{reveal:['次はオレだ。元気よくいこうぜ。','よし、次はオレの番だ！']},
    '伊達政宗':{reveal:['次は俺だ。主導権は譲らない。','次局は俺が相手だ。攻め合おう。']},
    'あすか':{reveal:['次はあたしよ。手加減しないから。','やっと出番ね。次はあたしが相手よ。']},
    'ユリア':{reveal:['次は私ですね。落ち着いて指しましょう。','次局、お相手します。よろしくお願いします。']},
    '玉ちゃん':{reveal:['次は玉ちゃんの番だよ。よろしくね。','次は私だよ。楽しく真剣にいこう。']},
    'まり':{reveal:['次は私だね。面白い将棋にしよう。','次は私が相手。思い切っていこう。']},
    'ぺんぺん':{reveal:['次はぺんぺんだよ。よろしく！','次はこっちの番。じっくりいこう。']},
    'げんどー':{reveal:['次は私が相手だ。始めよう。','次局はこちらが相手だ。']},
    '前田慶次':{reveal:['次は俺だ。派手に一局いこうか。','次局は俺が相手だ。思い切って来い。']},
    'シン':{reveal:['次は俺だ。ここから先は譲らない。','次局は俺が相手だ。速く仕掛けるぞ。']},
    'みさとさん':{reveal:['次は私ね。さあ、集中していこう。','次局は私が相手。気を引き締めていこう。']},
    'サウザー':{reveal:['次は俺だ。堂々と指してみせろ。','ここからは俺が相手だ。勝負所を逃すな。']},
    'リン':{reveal:['次は私だよ。がんばって指すね。','次は私の番だね。よろしく。']},
    'ラオウ':{reveal:['次は俺が相手だ。盤上で力を示せ。','次局、俺が受けて立つ。']},
    'カヲル':{reveal:['次は僕と指そう。楽しみにしているよ。','次の対局は僕だね。君の将棋を見せてほしい。']},
    '未来からやってきたみつき':{reveal:['次は未来の私が相手だよ。','ここまで来たね。次は未来の私と勝負。']}
  };
...
#tournament21540Panel #tourOpponentVoice21549.tourOpponentDialogue21592{border-color:#4f8dbd!important;background:linear-gradient(135deg,rgba(14,34,45,.98),rgba(8,20,17,.98))!important}#tournament21540Panel #tourOpponentVoice21549.tourOpponentDialogue21592 .tourDialoguePortrait{border-color:#67a9d8!important}
...
`;
  ...
  function lineFor(name,key){const p=OPPONENT_LINES[name]||{reveal:['次は私が相手です。よろしく。']},arr=p.reveal;let h=0;for(const c of String(key||name))h=(h*31+c.charCodeAt(0))>>>0;return arr[h%arr.length]}
  ...
  function syncOpponentDialogue(){
    const a=readActive(),host=document.getElementById('tourDialogue21547'),participant=document.getElementById('tourOpponentVoice21549'),opp=currentOpponentSlot();
    const bossStatus=a?.bossChallenge?.status,bossPhase=['pending','active','won','lost','draw'].includes(bossStatus),name=cleanName(opp);
    if(bossPhase||!opp||!name){if(host)host.style.removeProperty('display');participant?.classList.remove('tourOpponentDialogue21592');if(participant){delete participant.dataset.visualRole;const badge=participant.querySelector('.tourDialogueRole');if(badge&&participant.dataset.role)badge.textContent=participant.dataset.role}return null}
    if(host)host.style.setProperty('display','none','important');
    if(!participant)return null;
    participant.classList.add('tourOpponentDialogue21592');participant.dataset.visualRole='対戦相手';
    const badge=participant.querySelector('.tourDialogueRole');if(badge)badge.textContent='対戦相手';
    return participant;
  }
  function roundIntro(){
    ...
    const titles=['1回戦','準々決勝','準決勝','決勝'],subs=['ROUND OF 16','QUARTERFINAL','SEMIFINAL','FINAL'],src=portraitSrc(opp),rating=ratingOf(name),line=lineFor(name,key);
  ...
  function bossIntro(gate){
    ...
    const img=gate.querySelector('.tourAvatar img'),src=img?.currentSrc||img?.src||'',rating=ratingOf(boss),line=lineFor(boss,key+':boss');
