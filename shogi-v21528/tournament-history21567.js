/* Tournament recent-attempt history v2.15.67 + 21568 ordinal UX + 21570 game-card + 21571 road readability + 21573 history hierarchy + 21575 hero microtext + 21576 bracket status microtext + 21577 remaining microtext + 21578 resume affordance + 21579 completed retry
 * Display-only history UI. Reads existing tournament history; 21579 starts a fresh attempt only through the existing tournament API.
...
(function installTournamentAttemptHistory21567(){
  ...
  const fireBattleHidden=()=>{const a=active(),boss=a?.bossChallenge?.status||'locked';return !!(IS_FIRE_RUNTIME&&a&&!document.getElementById('tournament21540Panel')?.classList.contains('on')&&!a.pending&&!document.getElementById('resultBanner')?.classList.contains('on')&&(['active','draw','boss_active','boss_draw'].includes(a.status)||['active','draw'].includes(boss)))};
  ...
  const isCompleted=a=>!!a&&['champion','lost'].includes(a.status);
  const stateLabel=(a,current)=>!current?'挑戦':a?.status==='champion'?'優勝':a?.status==='lost'?'敗退':a?.status==='draw'?'指し直し':a?.pending==='next'?'次局待ち':'進行中';
  const resumeSelector=a=>{
    if(!a||isCompleted(a))return'';
  ...
  function retryCompleted(item){
    const a=active(),cupId=item?.dataset?.cupId||'';
    if(item?.dataset?.current!=='1'||item?.dataset?.retryable!=='1'||!isCompleted(a)||!cupId||a.cupId!==cupId)return false;
    try{return api()?.start?.(cupId,true)!==false}catch(e){return false}
  }
  function activateItem(item){return item?.dataset?.retryable==='1'?retryCompleted(item):resumeCurrent(item)}
...
#tournament21540Panel .tourAttemptHistoryItem21567.current[data-resumable="1"],#tournament21540Panel .tourAttemptHistoryItem21567.current[data-retryable="1"]{cursor:pointer}
#tournament21540Panel .tourAttemptHistoryItem21567.current[data-resumable="1"]:focus-visible,#tournament21540Panel .tourAttemptHistoryItem21567.current[data-retryable="1"]:focus-visible{outline:2px solid #f1ca6a;outline-offset:2px}
...
#tournament21540Panel .tourAttemptHistoryResume21578,#tournament21540Panel .tourAttemptHistoryRetry21579{flex:0 0 auto;color:#ffe49a;font-size:8px;font-weight:900}
...
`;
  ...
  function render(){
    ...
    const box=document.createElement('section');box.className='tourAttemptHistory21567';box.dataset.historyUi='21567';box.dataset.historyOrdinalUi='21568';box.dataset.historyHierarchyUi='21573';box.dataset.historyResumeUi='21578';box.dataset.historyRetryUi='21579';box.setAttribute('aria-label','最近の大会挑戦');
    ...
    rows.forEach(row=>{
      const current=row?.cupId===a.cupId&&Number(row?.startedAt)>0&&Number(row.startedAt)===Number(a.startedAt),resumable=current&&!!resumeSelector(a),retryable=current&&isCompleted(a),rating=Number(row?.rating),when=timeLabel(row?.startedAt),ordinal=attemptOrdinal(row),name=cupName(row?.cupId),actionLabel=retryable?'同じ杯に再挑戦':resumable?resumeLabel(a):'';
      const item=document.createElement('div');item.className='tourAttemptHistoryItem21567'+(current?' current':'');item.dataset.cupId=String(row?.cupId||'');item.dataset.current=current?'1':'0';item.dataset.resumable=resumable?'1':'0';item.dataset.retryable=retryable?'1':'0';item.dataset.attemptOrdinal=String(ordinal);item.setAttribute('aria-label',name+' '+ordinal+'回目 '+stateLabel(a,current)+(actionLabel?' '+actionLabel:''));
      if(resumable||retryable){item.setAttribute('role','button');item.tabIndex=0;item.title=actionLabel;item.addEventListener('click',()=>activateItem(item));item.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();activateItem(item)}})}
      ...
      const meta=document.createElement('span');meta.className='tourAttemptHistoryMeta21567';meta.appendChild(span('', 'R'+(Number.isFinite(rating)?rating:'—')));if(when)meta.appendChild(span('',when));meta.appendChild(span('tourAttemptHistoryState21567',stateLabel(a,current)));if(resumable)meta.appendChild(span('tourAttemptHistoryResume21578',resumeLabel(a)));if(retryable)meta.appendChild(span('tourAttemptHistoryRetry21579','再挑戦'));item.appendChild(meta);list.appendChild(item);
    });
  ...
  function install(){
    ...
    base.audit=()=>{render();const out=oldAudit(),box=document.querySelector('#tournament21540Panel .tourAttemptHistory21567'),items=box?[...box.querySelectorAll('.tourAttemptHistoryItem21567')]:[];return{...out,history21567:!!box,historyOrdinalUi21568:box?.dataset.historyOrdinalUi==='21568',historyHierarchyUi21573:box?.dataset.historyHierarchyUi==='21573',historyResumeUi21578:box?.dataset.historyResumeUi==='21578',historyRetryUi21579:box?.dataset.historyRetryUi==='21579',historyCount21567:items.length,historySourceCount21567:history().length,historyCurrent21567:items.filter(x=>x.dataset.current==='1').length,historyResumable21578:items.filter(x=>x.dataset.resumable==='1').length,historyRetryable21579:items.filter(x=>x.dataset.retryable==='1').length,historyResumeLabels21578:items.filter(x=>x.dataset.resumable==='1').map(x=>x.querySelector('.tourAttemptHistoryResume21578')?.textContent||''),historyRetryLabels21579:items.filter(x=>x.dataset.retryable==='1').map(x=>x.querySelector('.tourAttemptHistoryRetry21579')?.textContent||''),historyStateLabels21579:items.map(x=>x.querySelector('.tourAttemptHistoryState21567')?.textContent||''),historyItems21567:items.map(x=>x.dataset.cupId||''),historyAttemptOrdinals21568:items.map(x=>Number(x.dataset.attemptOrdinal)||0),historyOverflow21567:box?Math.max(0,box.scrollWidth-box.clientWidth):0}};
    render();return true;
  }
