...
`;
  ...
  function render(){
    ...
    rows.forEach(row=>{
      ...
      const item=document.createElement('div');item.className='tourAttemptHistoryItem21567'+(current?' current':'');item.dataset.cupId=String(row?.cupId||'');item.dataset.current=current?'1':'0';item.dataset.resumable=resumable?'1':'0';item.dataset.attemptOrdinal=String(ordinal);item.setAttribute('aria-label',name+' '+ordinal+'回目'+(current?' 進行中':''));
      if(resumable){item.setAttribute('role','button');item.tabIndex=0;item.title=resumeLabel(a);item.addEventListener('click',()=>resumeCurrent(item));item.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();resumeCurrent(item)}})}
