/* AI将棋先生 v2.15.33 prep1: 6〜10位を共通やねうら王＋水匠5へ段階接続するための検証レイヤー。公開対局の指し手はまだ変更しない。 */
(function installNext5YaneuraPrep21533(){
  ...
  function verifyRoster(){
    ...
    if(typeof C==='undefined'||!Array.isArray(C)){
      issues.push('C roster unavailable');
      return{ok:false,issues,targets:TARGETS.slice()};
    }
    for(const who of TARGETS){
      const exp=EXPECTED[who],row=C[who];
      if(!row){issues.push('missing index '+who);continue}
      if(row[0]!==exp.name)issues.push('name mismatch '+who+': '+row[0]+' != '+exp.name);
      if(Number(row[1])!==exp.rating)issues.push('rating mismatch '+who+': '+row[1]+' != '+exp.rating);
      if(who>=0&&who<=4)issues.push('top5 overlap '+who);
    }
    return{ok:issues.length===0,issues,targets:TARGETS.slice(),profiles:{...PROFILES}};
  }
  ...
  async function probe(state,who,options={}){
    ...
    const multiPV=Math.max(2,Math.min(8,Number(options.multiPV)||p.multiPV));
    await shared.init();
    ...
    return{
      ...
      bestToken:res?.move&&typeof usi==='function'?usi(res.move):'',
      info:res?.info||{},
      candidates,
      resign:!!res?.resign,
